...
var _ = Describe("Preheat with Manager", func() {
	...
	Context("10MiB file in cache", func() {
		...
		It("preheat files in cache should be ok", Label("preheat", "file", "cache"), func() {
			...
			out, err = preheatedSeedClient.Command("grep", "-E", "\"(put task to cache|put piece to cache|get piece from cache)\"", "/var/log/dragonfly/dfdaemon/dfdaemon.log").CombinedOutput()
			fmt.Println(err)
			Expect(err).NotTo(HaveOccurred())
			logs := string(out)
