...
func mockStoreHostScript(mock redismock.ClientMock) *redismock.ExpectedCmd {
	return mock.CustomMatch(matchScriptArgs).ExpectEvalSha("", []string{
		pkgredis.MakePersistentHostKeyInScheduler(1, mockRawHost.ID),
		pkgredis.MakePersistentHostsInScheduler(1),
	},
		mockRawHost.ID,
...
func mockDeleteHostScript(mock redismock.ClientMock, hostID string) *redismock.ExpectedCmd {
	return mock.CustomMatch(matchScriptArgs).ExpectEvalSha("", []string{
		pkgredis.MakePersistentHostKeyInScheduler(1, hostID),
		pkgredis.MakePersistentHostsInScheduler(1),
	}, hostID)
}
...
func TestHostManager_Load(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name: "redis error",
			mock: func(mock redismock.ClientMock) {
				mock.ExpectHGetAll(pkgredis.MakePersistentHostKeyInScheduler(1, mockRawHost.ID)).SetErr(errors.New("redis error"))
			},
			expect: func(t *testing.T, host *Host, loaded bool) {
				assert := assert.New(t)
				assert.False(loaded)
		...
		},
		{
			name: "host not found",
			mock: func(mock redismock.ClientMock) {
				mock.ExpectHGetAll(pkgredis.MakePersistentHostKeyInScheduler(1, mockRawHost.ID)).SetVal(map[string]string{})
			},
			expect: func(t *testing.T, host *Host, loaded bool) {
				assert := assert.New(t)
				assert.False(loaded)
		...
		},
		{
			...
			mock: func(mock redismock.ClientMock) {
				...
				mock.ExpectHGetAll(pkgredis.MakePersistentHostKeyInScheduler(1, mockRawHost.ID)).SetVal(fields)
			},
			expect: func(t *testing.T, host *Host, loaded bool) {
				assert := assert.New(t)
				assert.False(loaded)
		...
		},
		{
			...
			mock: func(mock redismock.ClientMock) {
				mock.ExpectHGetAll(pkgredis.MakePersistentHostKeyInScheduler(1, mockRawHost.ID)).SetVal(mockRawHostFields(mockRawHost.ID))
			},
			expect: func(t *testing.T, host *Host, loaded bool) {
				assert := assert.New(t)
				assert.True(loaded)
...
func TestHostManager_LoadAll(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name: "scan fails",
			mock: func(mock redismock.ClientMock) {
				mock.ExpectSScan(pkgredis.MakePersistentHostsInScheduler(1), 0, "*", 10).SetErr(errors.New("redis scan error"))
			},
			expect: func(t *testing.T, hosts []*Host, err error) {
				assert := assert.New(t)
				assert.Error(err)
		...
		},
		{
			...
			mock: func(mock redismock.ClientMock) {
				mock.ExpectSScan(pkgredis.MakePersistentHostsInScheduler(1), 0, "*", 10).SetVal([]string{"host1", "host2"}, 0)
				mock.ExpectHGetAll(pkgredis.MakePersistentHostKeyInScheduler(1, "host1")).SetVal(mockRawHostFields("host1"))
				mock.ExpectHGetAll(pkgredis.MakePersistentHostKeyInScheduler(1, "host2")).SetErr(errors.New("redis hgetall error"))
			},
			expect: func(t *testing.T, hosts []*Host, err error) {
				assert := assert.New(t)
				assert.NoError(err)
		...
		},
		{
			...
			mock: func(mock redismock.ClientMock) {
				mock.ExpectSScan(pkgredis.MakePersistentHostsInScheduler(1), 0, "*", 10).SetVal([]string{"host3"}, 123)
				mock.ExpectHGetAll(pkgredis.MakePersistentHostKeyInScheduler(1, "host3")).SetVal(mockRawHostFields("host3"))
				mock.ExpectSScan(pkgredis.MakePersistentHostsInScheduler(1), 123, "*", 10).SetVal([]string{"host4"}, 0)
				mock.ExpectHGetAll(pkgredis.MakePersistentHostKeyInScheduler(1, "host4")).SetVal(mockRawHostFields("host4"))
			},
			expect: func(t *testing.T, hosts []*Host, err error) {
				assert := assert.New(t)
				assert.NoError(err)
...
func TestHostManager_LoadRandom(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			mock: func(mock redismock.ClientMock) {
				mock.ExpectSMembers(pkgredis.MakePersistentHostsInScheduler(1)).SetErr(errors.New("redis error"))
			},
			expect: func(t *testing.T, hosts []*Host, err error) {
				assert := assert.New(t)
				assert.Error(err)
		...
		},
		{
			...
			mock: func(mock redismock.ClientMock) {
				mock.ExpectSMembers(pkgredis.MakePersistentHostsInScheduler(1)).SetVal([]string{"host1", "host2", "host3"})
			},
			expect: func(t *testing.T, hosts []*Host, err error) {
				assert := assert.New(t)
				assert.NoError(err)
		...
		},
		{
			...
			mock: func(mock redismock.ClientMock) {
				mock.ExpectSMembers(pkgredis.MakePersistentHostsInScheduler(1)).SetVal([]string{"host1", "host2"})
				mock.ExpectHGetAll(pkgredis.MakePersistentHostKeyInScheduler(1, "host2")).SetVal(mockRawHostFields("host2"))
			},
			expect: func(t *testing.T, hosts []*Host, err error) {
				assert := assert.New(t)
				assert.NoError(err)
		...
		},
		{
			...
			blocklist: set.NewSafeSet[string](),
			mock: func(mock redismock.ClientMock) {
				mock.ExpectSMembers(pkgredis.MakePersistentHostsInScheduler(1)).SetVal([]string{"host1"})
				mock.ExpectHGetAll(pkgredis.MakePersistentHostKeyInScheduler(1, "host1")).SetErr(errors.New("redis hgetall error"))
			},
			expect: func(t *testing.T, hosts []*Host, err error) {
				assert := assert.New(t)
				assert.NoError(err)
...
func TestHostManager_RunGC(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name: "load all fails",
			mock: func(mock redismock.ClientMock) {
				mock.ExpectSScan(pkgredis.MakePersistentHostsInScheduler(1), 0, "*", 10).SetErr(errors.New("redis error"))
			},
			expect: func(t *testing.T, err error) {
				assert := assert.New(t)
				assert.Error(err)
		...
		},
		{
			...
			mock: func(mock redismock.ClientMock) {
				...
				mock.ExpectSScan(pkgredis.MakePersistentHostsInScheduler(1), 0, "*", 10).SetVal([]string{"host1"}, 0)
				mock.ExpectHGetAll(pkgredis.MakePersistentHostKeyInScheduler(1, "host1")).SetVal(fields)
			},
			expect: func(t *testing.T, err error) {
				assert := assert.New(t)
				assert.NoError(err)
