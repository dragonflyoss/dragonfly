...
func TestTaskIDV2ByURLBased(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expect: func(t *testing.T, d string) {
				assert := assert.New(t)
				assert.Equal("5844f27a257287e9b734256bb25603d8005422ced8c0377f15063ec11963b25f", d)
			},
		},
		{
			...
			expect: func(t *testing.T, d string) {
				assert := assert.New(t)
				assert.Equal("06408fbf247ddaca478f8cb9565fe5591c28efd0994b8fea80a6a87d3203c5ca", d)
			},
		},
		{
			name: "generate taskID with tag",
			...
			expect: func(t *testing.T, d string) {
				assert := assert.New(t)
				assert.Equal("3c3f230ef9f191dd2821510346a7bc138e4894bee9aee184ba250a3040701d2a", d)
			},
		},
		{
			...
			expect: func(t *testing.T, d string) {
				assert := assert.New(t)
				assert.Equal("c9f9261b7305c24371244f9f149f5d4589ed601348fdf22d7f6f4b10658fdba2", d)
			},
		},
		{
			...
			expect: func(t *testing.T, d string) {
				assert := assert.New(t)
				assert.Equal("9f7c9aafbc6f30f8f41a96ca77eeae80c5b60964b3034b0ee43ccf7b2f9e52b8", d)
			},
		},
		{
			...
			expect: func(t *testing.T, d string) {
				assert := assert.New(t)
				assert.Equal("457b4328cde278e422c9e243f7bfd1e97f511fec43a80f535cf6b0ef6b086776", d)
			},
		},
		{
			...
			expect: func(t *testing.T, d string) {
				assert := assert.New(t)
				assert.Equal("b171331534b80e0bf91da38ebbfcdbf4d177898f4b9beac44f14733e3f004d4e", d)
			},
		},
		{
			...
			expect: func(t *testing.T, d string) {
				assert := assert.New(t)
				assert.Equal("8b3f6e9b9b8fe20903bced565cfd1d0aaef354a4c17573f0c2c1979210443f9d", d)
			},
		},
		{
			...
			expect: func(t *testing.T, d string) {
				assert := assert.New(t)
				assert.Equal("7c8801d0596be5e8f9449d5c4af23866c72fe5205119c0e5912981f3b16a37aa", d)
			},
		},
		{
			...
			expect: func(t *testing.T, d string) {
				assert := assert.New(t)
				assert.Equal("6196a6846023f6d3c1e4d30f6c86f3d4186e4c664a33e5692b0e04e49b26a9af", d)
			},
		},
		{
			...
			expect: func(t *testing.T, d string) {
				assert := assert.New(t)
				assert.Equal("c8f4b41117329d54af920010394f6f607bac707e933ab2f18d372e3dd4c7fcb3", d)
			},
		},
		{
			...
			expect: func(t *testing.T, d string) {
				assert := assert.New(t)
				assert.Equal("980ee327518ccc5a7c30703e1a2232e8ba9047b39431f940636c85b6146f8b9a", d)
			},
		},
	}
...
func TestTaskIDV2ByContent(t *testing.T) {
	assert := assert.New(t)
	assert.Equal("e2d0fe1585a63ec6009c8016ff8dda8b17719a637405a4e23c0ff81339148249", TaskIDV2ByContent("This is a test file"))
}

func TestPersistentCacheTaskIDbyContent(t *testing.T) {
	assert := assert.New(t)
	assert.Equal("e2d0fe1585a63ec6009c8016ff8dda8b17719a637405a4e23c0ff81339148249", PersistentCacheTaskIDByContent("This is a test file"))
}
	...
	tests := []struct {
	...
	}{
		{
			...
			expect: func(t *testing.T, d string, err error) {
				assert := assert.New(t)
				assert.NoError(err)
				assert.Equal("b2c366cce7e68013d5441c6326d5a3e1b12aeb5ed58564d0fd3fa089bc29cb6e", d)
			},
		},
		{
			...
			expect: func(t *testing.T, d string, err error) {
				assert := assert.New(t)
				assert.NoError(err)
				assert.Equal("94381a28e8c039fedfa78de025158a068226c3ccd041b22c2c8e73fc993584e9b167d9ae32bc8b372c66701c808ab134e0768c8f16b9a3e61eec1ccf8faa9db8", d)
			},
		},
		{
	...
	tests := []struct {
	...
	}{
		{
			...
			expect: func(t *testing.T, d string, err error) {
				assert := assert.New(t)
				assert.NoError(err)
				assert.Equal(TaskIDV2ByContent("This is a content"), d)
			},
		},
		{
		...
		},
		{
			...
			expect: func(t *testing.T, d string, err error) {
				assert := assert.New(t)
				assert.NoError(err)
				assert.Equal(TaskIDV2ByURLBased("http://registry.example.com/v2/library/ubuntu/manifests/sha256:b2c366cce7e68013d5441c6326d5a3e1b12aeb5ed58564d0fd3fa089bc29cb6e", nil, "foo", "bar", nil, ""), d)
			},
		},
		{
			...
			expect: func(t *testing.T, d string, err error) {
				assert := assert.New(t)
				assert.NoError(err)
				assert.Equal(TaskIDV2ByURLBased("http://registry.example.com/v2/library/ubuntu/manifests/latest", nil, "foo", "bar", nil, ""), d)
			},
		},
		{
			...
			expect: func(t *testing.T, d string, err error) {
				assert := assert.New(t)
				assert.NoError(err)
				assert.Equal(TaskIDV2ByURLBased("http://registry.example.com/v2/library/ubuntu/blobs/sha256:b2c366cce7e68013d5441c6326d5a3e1b12aeb5ed58564d0fd3fa089bc29cb6e", nil, "foo", "bar", nil, ""), d)
			},
		},
		{
			...
			expect: func(t *testing.T, d string, err error) {
				assert := assert.New(t)
				assert.NoError(err)
				assert.Equal(TaskIDV2ByURLBased("https://example.com/file.txt", nil, "foo", "bar", nil, ""), d)
			},
		},
		{
			name:                        "unsupported digest algorithm",
