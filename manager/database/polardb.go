...
func newPolardb(cfg *config.Config) (*gorm.DB, error) {
	polardbCfg := &cfg.Database.Polardb

	// Format dsn string.
	dsn := formatPolardbDSN(polardbCfg)
	...
	// Connect to polardb using polardbx driver (MySQL compatible).
...
func formatPolardbDSN(cfg *config.PolardbConfig) string {
	return cfg.User + ":" + cfg.Password + "@tcp(" + cfg.AddrList + ")/" + cfg.DBName + "?charset=utf8mb4&parseTime=True&loc=Local"
}
